...
const RoomDetails = () => {
  ...
  // OnSubmit Handler function to check the availability and book the room
  const onSubmitHandler = async (e) => {
    try {
      ...
      if(!isAvailable) {
        ...
        const {data} = await axios.post(`/api/bookings/book`, {room: id, checkInDate, checkOutDate, guests, paymentMethod: "Pay at Hotel"}, {headers: {Authorization: `Bearer ${await getToken()}`}, withCredentials: true});
  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
      ...
      <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] rounded-xl mx-auto p-6 mt-16 max-w-6xl'>
      ...
      <div className='flex flex-col items-start gap-4'>
        ...
        <button className='px-6 py-2.5 mt-4 rounded text-white bg-primary hover:bg-primary-dull transition-all cursor-pointer active:scale-95'>Contact Now</button> 
      </div>
