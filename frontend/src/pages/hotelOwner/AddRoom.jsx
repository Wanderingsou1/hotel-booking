...
const AddRoom = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      // Convert Amenities to Array and keep only true values
      ...
      <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        {Object.keys(images).map((key) => (
          <label htmlFor={`roomImage${key}`} key={key}>
            ...
            <input type="file" accept='image/*' id={`roomImage${key}`} className='hidden'  onChange={(e) => setImages({...images, [key]: e.target.files[0]})}/>
          </label>
        ))}
      </div>
