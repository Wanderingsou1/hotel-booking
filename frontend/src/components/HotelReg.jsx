      ...
      <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className='flex bg-white rounded-xl max-w-4xl max-md:mx-2'>
        ...
        <div className='relative flex flex-col items-center md:w-1/2 p-8 md:p-10'>
          ...
          <div className='w-full mt-4'>
            ...
            <input type="text"  onChange={(e) => setName(e.target.value)} value={name} id='name' placeholder='Type here' className='w-full border border-gray-200 rounded px-3 py-2.5 mt-1 outline-indigo-500 font-light' required/>
          </div>

          {/* Phone */}
          <div className='w-full mt-4'>
            <label htmlFor="contact" className='font-medium text-gray-500'>Phone</label>
            <input type="text"  onChange={(e) => setContact(e.target.value)} value={contact} id='contact' placeholder='Type here' className='w-full border border-gray-200 rounded px-3 py-2.5 mt-1 outline-indigo-500 font-light' required/>
          </div>

          {/* Address */}
          <div className='w-full mt-4'>
            <label htmlFor="address" className='font-medium text-gray-500'>Address</label>
            <input type="text"  onChange={(e) => setAddress(e.target.value)} value={address} id='address' placeholder='Type here' className='w-full border border-gray-200 rounded px-3 py-2.5 mt-1 outline-indigo-500 font-light' required/>
          </div>
